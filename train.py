...
from model.transformer import Transformer, MemoryAugmentedEncoder, MeshedDecoder, PriorKnowledgeAugmentedAttention
import torch
...
def load_descriptions(file_path):
    ...
    # 将描述按每5条分组，代表同一张图像
    grouped_descriptions = [descriptions[i:i + 5] for i in range(0, len(descriptions), 5)]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='SFDR')
    ...
    parser.add_argument('--fused_features_path', type=str,
                        default='./datasets/Sydney_Captions/features/fused_feature/fusion')
    parser.add_argument('--roi_features_path', type=str,
                        default='./datasets/Sydney_Captions/features/roi_feature/roi')


    parser.add_argument('--logs_folder', type=str, default='log_record')
    ...
    image_field = ImageDetectionsField(max_detections=50, load_in_tmp=False,
                                       scene_detections_path=args.fused_features_path,
                                       object_detections_path=args.roi_features_path)
    ...
    if args.resume_last or args.resume_best:
        if args.resume_last:
            fname = 'checkpoints/%s_last.pth' % args.exp_name
        else:
            fname = 'checkpoints/%s_best.pth' % args.exp_name
    ...
    for e in range(start_epoch, start_epoch + 50):
        ...
        if switch_to_rl and not best:
            data = torch.load('checkpoints/%s_best.pth' % args.exp_name)
            torch.set_rng_state(data['torch_rng_state'])
            torch.cuda.set_rng_state(data['cuda_rng_state'])
            np.random.set_state(data['numpy_rng_state'])
            random.setstate(data['random_rng_state'])
        ...
        torch.save({
            ...
            'use_rl': use_rl,
        }, 'checkpoints/%s_last.pth' % args.exp_name)

        if best:
            copyfile('checkpoints/%s_last.pth' % args.exp_name, 'checkpoints/%s_best.pth' % args.exp_name)
