...
# 如果想要保存推理的caption生成结果，使用这段代码，并把上面那段注释掉即可
# def predict_captions(model, dataloader, text_field, output_file='generated_descriptions.txt'):
#     import itertools
#     model.eval()
#     gen = {}
#     gts = {}
...
#
#                 with torch.no_grad():
#                     out, _ = model.beam_search(detections, detections_gl, 20, text_field.vocab.stoi['<eos>'], 5,
#                                                out_size=1)
#                 caps_gen = text_field.decode(out, join_words=False)
#
#                 for i, (gts_i, gen_i) in enumerate(zip(caps_gt, caps_gen)):
...
#                     f_out.write(f'Generated: {gen_i}\n')
#                     f_out.write(f'Ground Truth: {gts_i}\n\n')
#
#                 pbar.update()
#
#         gts = evaluation.PTBTokenizer.tokenize(gts)
...
if __name__ == '__main__':
    ...
    data = torch.load('./checkpoints/Sydney_best.pth')
